...
public record ContactDto(string Headline, string Body);

public record TravelHighlightDto(string Destination, string Caption, string? ImageUrl);
...
public record CollaborationDto(
    string BrandName, string Description, string? Testimonial, string? LogoUrl, string[] CampaignImageUrls);
